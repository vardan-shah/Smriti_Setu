...
import { Coffee, Music, TreePine, Mountain, Moon, Sun, Home as HomeIcon, Droplet, Pill, Volume2, VolumeX, Wifi, WifiOff } from "lucide-react";
...
const translations: any = {
  English: {
    ...
    voiceOnline: "Voice Active", voiceOffline: "Voice Disabled (Offline)"
  },
  Assamese: {
    ...
    voiceOnline: "ভইচ সক্ৰিয়", voiceOffline: "ভইচ নিষ্ক্ৰিয় (অফলাইন)"
  },
  Bengali: {
    ...
    voiceOnline: "ভয়েস সক্রিয়", voiceOffline: "ভয়েস অক্ষম (অফলাইন)"
  },
  Hindi: {
    ...
    voiceOnline: "आवाज़ सक्रिय", voiceOffline: "आवाज़ अक्षम (ऑफ़लाइन)"
  }
};
...
function GameBoard() {
  ...
  const CARDS = [
    { id: 1, icon: Coffee, key: "tea", color: "bg-amber-100 text-amber-700" },
    { id: 2, icon: Music, key: "music", color: "bg-rose-100 text-rose-700" },
    { id: 3, icon: TreePine, key: "bamboo", color: "bg-emerald-100 text-emerald-700" },
    { id: 4, icon: Mountain, key: "hills", color: "bg-slate-200 text-slate-700" },
    { id: 5, icon: Sun, key: "morning", color: "bg-orange-100 text-orange-600" },
    { id: 6, icon: Moon, key: "night", color: "bg-indigo-100 text-indigo-700" },
  ];
  ...
  // Simulating Bhashini API call (degrades gracefully if offline)
  const playVoicePrompt = (text: string) => {
    if (!isOnline) return; // Dual Mode: Fail gracefully if offline
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      const utterance = new SpeechSynthesisUtterance(text);
      // Map to available browser languages (fallback for demo)
      utterance.lang = lang === "Hindi" ? "hi-IN" : lang === "Bengali" ? "bn-IN" : "en-IN";
      utterance.rate = 0.85; // Speak slower for elderly users
      console.log("Speaking: " + text); window.speechSynthesis.speak(utterance);
    }
  };
  ...
  const handleCardClick = (index: number) => {
    ...
    // Play voice prompt when card is revealed (if online)
    ...
    if (newFlipped.length === 2) {
      ...
      if (first.key === second.key) {
        ...
        if (matched.length + 1 === CARDS.length) {
          setWin(true);
          saveSessionLocally({ game: "matching", matches: CARDS.length, timeSpent: 120, langUsed: lang });
        }
      } else {
  ...
  return (
    <div className="w-full max-w-5xl flex flex-col md:flex-row gap-8">
      ...
      {/* SIDEBAR: REMINDERS & CLOUD STATUS */}
      <div className="w-full md:w-1/3 space-y-6">
        
        {/* DUAL MODE INDICATOR */}
        <div className={`rounded-3xl p-6 shadow-md border-4 flex items-center gap-4 transition-colors ${isOnline ? 'bg-blue-50 border-blue-100 text-blue-700' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
          {isOnline ? <Wifi className="w-10 h-10 shrink-0" /> : <WifiOff className="w-10 h-10 shrink-0" />}
          <div>
            <h3 className="font-bold text-xl leading-tight">Cloud Integration</h3>
            <p className="text-sm font-medium opacity-80 flex items-center gap-1 mt-1">
              {isOnline ? <Volume2 className="w-4 h-4"/> : <VolumeX className="w-4 h-4"/>}
              {isOnline ? t.voiceOnline : t.voiceOffline}
            </p>
          </div>
        </div>
